...
function extractCredentials (done) {
  ...
  // Scope
  var scope = this.req.body && this.req.body.scope;
  this.newScope = [];
  this.oldScope = [];
  if (scope) {
    if (scope.match('^[a-zA-Z0-9-_:]*[a-zA-Z0-9-_]{1}$')) {
      this.newScope = scope.split(":");
    } else {
...
function checkScope (done) {
    var self = this;
    return this.model.checkScope(this.client.clientId, this.grantType, 
        this.newScope, this.oldScope, function(err, scope) {
        if (err) {
            return done(error('invalid_request',
                'Invalid scope parameter or parameter missing'));
        }
        self.newScope = scope;
        done();
    });
...
function useRefreshTokenGrant (done) {
  ...
  this.model.getRefreshToken(token, function (err, refreshToken) {
    ...
    if (refreshToken.scope) {
        self.oldScope = refreshToken.scope;
    }
...
function saveAccessToken (done) {
  ...
  this.model.saveAccessToken(accessToken, this.client.clientId, expires,
      this.user, this.newScope, function (err) {
    if (err) return done(error('server_error', false, err));
    done();
  });
}
